...
public class PathCacheExample {
	...
	private static void list(CuratorFramework client, PathChildrenCache cache) {
		if (cache.getCurrentData().size() == 0) {
			...
			for (ChildData data : cache.getCurrentData()) {
				...
				try {
					List<String> children = client.getChildren().forPath(data.getPath());

					for (String child : children) {
						String fullPath = data.getPath() + "/" + child;
